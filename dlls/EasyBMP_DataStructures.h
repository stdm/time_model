...
#include "EasyBMP_Api.h" //by thilo to compile as dll in msvc++

inline double EASYBMP_API Square( double number )
{ return number*number; }

inline int EASYBMP_API IntSquare( int number )
{ return number*number; }
#endif

int EASYBMP_API IntPow( int base, int exponent );
...
inline bool EASYBMP_API IsBigEndian()
{
 short word = 0x0001;
 if((*(char *)& word) != 0x01 )
 { return true; }
...
inline ebmpWORD EASYBMP_API FlipWORD( ebmpWORD in )
{ return ( (in >> 8) | (in << 8) ); }

inline ebmpDWORD EASYBMP_API FlipDWORD( ebmpDWORD in )
{
